...
class ToolOrchestrator(
    ...
    suspend fun execute(rawJson: String): ToolResult = runCatching {
        ...
        when (name) {
            "open_app" -> {
                val packageName = args["package_name"]?.jsonPrimitive?.content ?: error("package_name")
                val launchIntent = context.packageManager.getLaunchIntentForPackage(packageName)
                    ?: return@runCatching ToolResult(false, "App not installed or launchable: $packageName")
                ...
                when (name) {
                    "click_text" -> ToolResult(service.clickText(args["text"]?.jsonPrimitive?.content ?: error("text"), args["exact"]?.jsonPrimitive?.content?.toBoolean() == true), "click_text")
